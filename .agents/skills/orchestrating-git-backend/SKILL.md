...
description: "Injects the invariants of the GitCraque Node backend under server/**: the boot-time route contract, the single git spawn point, the non-reentrant mutation lock, i18n error keys and the security guards. Use whenever a task touches server/**, adds or changes a REST route or WebSocket event, runs a git command from Node, parses git output, or edits contract.mjs — even when the user never mentions skills, the backend, or a file name."
metadata:
  type: task
